...
**Status:** Accepted 2026-08-30 — nothing here blocks scaffolding · **Stage:** Authentication, invitations, platform-led onboarding, staff presence, events, atmosphere, feed, booking enquiries, offers, and the mobile-first UI foundation · **Last updated:** 2026-09-06

This document describes the technical architecture. The foundation schema, RLS, authentication, invitation acceptance, actor resolution, application `can()`, platform-led onboarding, staff presence, events, atmosphere, the venue feed, booking enquiries and **offers** now exist. Remaining product modules (analytics UI, notifications, media uploads) are not built. Product scope is in [product-brief.md](./product-brief.md), the schema in [data-model.md](./data-model.md), authorisation in [roles-and-permissions.md](./roles-and-permissions.md), authentication in [authentication.md](./authentication.md), and the reasoning and unresolved questions in [decisions-and-open-questions.md](./decisions-and-open-questions.md).
...
- **Isolated automated tests:** local database and browser verification uses a separate Docker project `venuboard-test` generated from the same repository migrations and seed. It does not reset ordinary development project `venuboard`. Commands: `test:stack:*`, `test:db`, `test:e2e`, `test:verify` (database/types/Playwright only; format/lint/unit/build remain `npm run verify`).
