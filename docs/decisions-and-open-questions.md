...
- **ADR** — an architecture decision record. `Accepted` means agreed and safe to build on. `Proposed` means _recommended, not yet agreed_.
...
### ADR-005 — Tenant isolation through RLS _and_ application authorisation
...
**Decision:** Users join a business or venue only through a tokenised, expiring, single-use invitation that carries the target scope and role. Nobody may invite above their own level, and venue-manager invitation rights are off unless the business owner enables them. **Amended:** because there is no public self-service signup in the MVP (ADR-033), the _first_ business owner of a new business is always created by the platform operator.
...
**Consequences:** A venue is publishable within minutes of creation. Slug changes need redirect handling, and the global uniqueness of slugs is a minor product constraint that the admin UI must explain clearly. The request proxy (`src/proxy.ts`) and the domains table are now safe to build.
...
**Consequences:** Nightlife venues get an appropriate presentation without the platform becoming an adult site. The classification field, the platform lock and the takedown mechanism can now be built. **An age notice is not moderation and not legal compliance** — a moderation _workflow_, acceptable-use policy and takedown _process_ are required before launch and remain unresolved (OQ-07, OQ-08), confirmed as launch blockers rather than scaffolding blockers. ADR-036 gives the operator the lever; it does not give them a process.
**History:** proposed 2026-08-29; accepted 2026-08-30.
...
**Scaffold note (2026-08-30):** the initial application scaffold recorded the generated versions in `package.json` and `package-lock.json`, and used **npm** as the installer (`packageManager: npm@10.8.2`). That is a working choice, not an accepted lock. This record stays `Proposed` so versions and the package manager can still be changed without a formal reversal.
...
**Status:** **Accepted 2026-08-30**
**Context:** Prohibited content will eventually be published by a venue, and the operator carries the legal and reputational exposure. Waiting to open a support session before taking illegal content down is the wrong trade, but a platform action that can _write_ tenant content would undermine the whole support model.
...
| **OQ-37** | Which fictional venue names, brands and placeholder image sources does the seed dataset use?                                   | Must be plainly fictional and must not resemble real venues in the target cities (ADR-035). Does not block the scaffold: the dataset's _shape_ is specified, only the invented names are outstanding                                                               |
...
Two questions from the first revision were **partially** resolved and remain open in narrowed form: OQ-05 (plan _structure_ settled by ADR-008; **price points** still open) and OQ-04 (data _location_ settled by ADR-032; **compliance obligations** still open).
...
**Working choices recorded, not accepted as ADRs:**

- **npm** is the current package manager (`package-lock.json`, `packageManager` field). Changeable; no package-manager ADR exists.
- Generated runtime versions live in `package.json`. [ADR-026](#adr-026--no-pinned-package-versions-yet) remains `Proposed`.
- Interface locales `en` and `th` with `localePrefix: "always"`, default `en`, matching [ADR-015](#adr-015--next-intl-for-english-and-thai) which remains `Proposed`.
- Local Supabase CLI configuration only (`project_id = "venuboard"`). No remote project, no region project, no link.

**Not decided by this scaffold:** anything in [section 4.1](#41-obligations-on-the-first-implementation), [4.2](#42-launch-blockers--required-before-production-not-before-code) or [4.3](#43-feature-specific-decisions--required-before-the-feature-not-before-the-scaffold).
